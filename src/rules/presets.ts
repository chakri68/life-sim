...
const EMPTY_SPECIES: SpeciesParams = {
  mobile: false, diet: [], breedTime: 0, metabolism: 0, startEnergy: 0, gain: 0, seedDensity: 0,
};
...
function plant(seedDensity: number, breedTime = 3): SpeciesParams {
  return { mobile: false, diet: [], breedTime, metabolism: 0, startEnergy: 1, gain: 0, seedDensity };
}

/** A mobile consumer that eats the given species. */
function animal(
  diet: number[],
  o: { breedTime: number; startEnergy: number; gain: number; seedDensity: number; metabolism?: number; moveProb?: number; huntSuccess?: number; huntThreshold?: number; maxAge?: number },
): SpeciesParams {
  return {
    mobile: true,
...
function predatorPrey(): Config {
  return ecosystem('Predator-Prey (WaTor)', [
    // Fish: mobile but never starves; breeds on a timer (no diet).
    { state: { name: 'Fish', color: '#3b82f6' },
      species: { mobile: true, diet: [], breedTime: 4, metabolism: 0, startEnergy: 1, gain: 0, seedDensity: 0.32 } },
    // Shark (state 2) eats fish (state 1).
    { state: { name: 'Shark', color: '#e5484d' },
      species: animal([1], { breedTime: 12, startEnergy: 12, gain: 4, seedDensity: 0.06 }) },
  ], { type: 'vonneumann', radius: 1 }); // classic WaTor uses 4-neighbor adjacency
}
...
function foodChain(): Config {
  return ecosystem('Food Chain · grass→rabbit→fox', [
    ...
    { state: { name: 'Rabbit', color: '#d8c79a' },                                   // 2
      species: animal([1], { breedTime: 6, startEnergy: 8, gain: 4, seedDensity: 0.12 }) },
    { state: { name: 'Fox', color: '#e8833a' },                                      // 3
      species: animal([2], { breedTime: 14, startEnergy: 16, gain: 8, seedDensity: 0.04 }) },
  ]);
}
...
function twoPrey(): Config {
  return ecosystem('Two Prey · rabbit & mouse vs fox', [
    ...
    { state: { name: 'Rabbit', color: '#d8c79a' },                                    // 2
      species: animal([1], { breedTime: 6, startEnergy: 8, gain: 4, seedDensity: 0.1 }) },
    { state: { name: 'Mouse', color: '#9aa0a6' },                                     // 3 (breeds faster, weaker)
      species: animal([1], { breedTime: 4, startEnergy: 6, gain: 3, seedDensity: 0.1 }) },
    { state: { name: 'Fox', color: '#e8833a' },                                       // 4 eats both
      species: animal([2, 3], { breedTime: 14, startEnergy: 16, gain: 8, seedDensity: 0.03 }) },
  ]);
}
...
function foodWeb(): Config {
  return ecosystem('Food Web · grass→rabbit→hawk · grass→deer→wolf', [
    ...
    { state: { name: 'Deer', color: '#b07a4f' },                                      // 3
      species: animal([1], { breedTime: 8, startEnergy: 14, gain: 7, seedDensity: 0.07 }) },
    { state: { name: 'Wolf', color: '#c6cbd4' },                                      // 4 hunts deer
      species: animal([3], { breedTime: 14, startEnergy: 18, gain: 10, seedDensity: 0.03 }) },
    { state: { name: 'Hawk', color: '#8b5cf6' },                                      // 5 hunts rabbit
      species: animal([2], { breedTime: 11, startEnergy: 16, gain: 9, seedDensity: 0.04 }) },
  ]);
}
...
function reactionPreset(name: string, feed: number, kill: number, colors: string[]): Config {
  return {
    name,
    width: RD,
    height: RD,
...
const coral = () => reactionPreset('Coral (Gray-Scott)', 0.0545, 0.062, RAMP_TEAL);
const mitosis = () => reactionPreset('Mitosis · dividing spots', 0.0367, 0.0649, RAMP_VIOLET);
...
export const PRESETS: Preset[] = [
  ...
  { id: 'food-chain', label: 'Food Chain (grass→rabbit→fox)', build: foodChain },
...
export const REACTION_PATTERNS: Array<{ label: string; feed: number; kill: number }> = [
