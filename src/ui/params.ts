...
function slider(
  ...
  const atMax =
    def.sentinel !== undefined &&
    (stored === undefined || stored >= def.sentinel);
  ...
  const showValue = (raw: number) => {
    ...
    else if (def.zeroLabel !== undefined && raw === 0) {
      text = def.zeroLabel;
      warn = def.zeroWarn ?? false;
    }
  ...
  const input = el("input", {
    ...
    oninput: (e: Event) => {
      ...
      sp[def.key] =
        def.sentinel !== undefined && raw === def.max
          ? def.sentinel
          : raw / scale;
      showValue(raw);
      onChange();
...
function dietChips(
  ...
  const wrap = el("div", { class: "diet-chips" }, [
    el("span", { class: "muted diet-label" }, "eats:"),
  ]);
  let any = false;
...
export function renderEcosystem(
  ...
  container.append(
    el(
      "p",
      { class: "muted" },
      "Immobile species are producers (spread without food). Mobile species hunt their diet, gain energy, and starve when metabolism drains them to zero.",
    ),
  );
