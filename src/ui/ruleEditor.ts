...
function opSelect(
  value: CompareOp,
  onChange: (v: CompareOp) => void,
): HTMLSelectElement {
  const sel = el("select", {
    class: "op-select",
    onchange: (e: Event) =>
      onChange((e.target as HTMLSelectElement).value as CompareOp),
  });
...
function conditionRow(
  ...
  const rhsValue =
    cond.rhs.kind === "const"
      ? el("input", {
          ...
          oninput: (e: Event) => {
            if (cond.rhs.kind === "const")
              cond.rhs.value = Number((e.target as HTMLInputElement).value);
            onCommit();
          },
        })
  ...
  return el("div", { class: "cond-row" }, [
    ...
    stateSelect(cfg, cond.state, false, (v) => {
      cond.state = v;
      onCommit();
    }),
    opSelect(cond.op, (v) => {
    ...
    el(
      "button",
      { class: "icon-btn", title: "Remove condition", onclick: onRemove },
      "×",
    ),
  ]);
}
...
function ruleRow(
  ...
  rule.conditions.forEach((cond) => {
    conds.append(
      conditionRow(
        ...
        onCommit,
        () => {
          rule.conditions = rule.conditions.filter((c) => c !== cond);
          rerender();
        },
        rerender,
      ),
  ...
  return el("div", { class: "rule" }, [
    el("div", { class: "rule-head" }, [
      ...
      stateSelect(cfg, rule.when, true, (v) => {
        rule.when = v;
        onCommit();
      }),
      ...
      el(
        "button",
        { class: "icon-btn", title: "Move up", onclick: () => move(-1) },
        "↑",
      ),
      el(
        "button",
        { class: "icon-btn", title: "Move down", onclick: () => move(1) },
        "↓",
      ),
      el(
        "button",
    ...
    el("div", { class: "rule-result" }, [
      ...
      stateSelect(cfg, rule.become, false, (v) => {
        rule.become = v;
        onCommit();
      }),
      ...
      el("input", {
        ...
        oninput: (e: Event) => {
          rule.prob = Number((e.target as HTMLInputElement).value);
          onCommit();
        },
      }),
    ]),
...
export function renderRules(
  ...
  if (cfg.rules.length === 0) {
    container.append(
      el("p", { class: "muted" }, "No rules yet. Cells will hold their state."),
    );
  }
  cfg.rules.forEach((rule, i) =>
    container.append(ruleRow(cfg, rule, i, onCommit, rebuild)),
  );
  container.append(
    el(
      "button",
