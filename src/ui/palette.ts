...
export function renderPalette(container: HTMLElement, cfg: Config, h: PaletteHandlers): void {
  clear(container);
  cfg.states.forEach((state, i) => {
    const swatch = el('input', {
      ...
      type: 'color',
      value: state.color,
      oninput: (e: Event) => { state.color = (e.target as HTMLInputElement).value; h.commit(); },
    });
    const name = el('input', {
      class: 'state-name',
      type: 'text',
      value: state.name,
      oninput: (e: Event) => { state.name = (e.target as HTMLInputElement).value; h.commit(); },
    });
    const row = el('div', {
  ...
  container.append(
    el('button', { class: 'add-rule', onclick: () => h.addState() }, '+ Add state'),
  );
}
