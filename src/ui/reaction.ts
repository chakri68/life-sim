...
export function renderReaction(
  ...
  container.append(
    el(
      "p",
      { class: "muted" },
      "Two chemicals diffuse and react. Tiny feed/kill changes flip between coral, mazes, spots and waves. Paint on the canvas to inject more.",
    ),
  );
    ...
    chips.append(
      el(
        "button",
        {
          class:
            "chip" +
            (Math.abs(p.feed - pat.feed) < 1e-6 &&
            Math.abs(p.kill - pat.kill) < 1e-6
              ? " on"
              : ""),
          onclick: () => {
            p.feed = pat.feed;
            ...
            hooks.reseed();
            hooks.commit();
            renderReaction(container, cfg, hooks);
          },
        },
        pat.label,
      ),
  ...
  container.append(
    el("div", { class: "param-row" }, [el("label", {}, "pattern"), chips]),
  );
  ...
  p.colors.forEach((color, i) => {
    swatches.append(
      el("input", {
        ...
        value: color,
        oninput: (e: Event) => {
          p.colors[i] = (e.target as HTMLInputElement).value;
          hooks.applyColormap();
          hooks.commit();
        },
      }),
  ...
  container.append(
    el("div", { class: "param-row" }, [el("label", {}, "colormap"), swatches]),
  );
}
